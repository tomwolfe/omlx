...
from omlx.config_quant import QuantizationConfig
...
def universal_quant_predicate(
    path: str, module, config: dict | QuantizationConfig, oq_level: int = 4
) -> Union[bool, dict]:
    ...
    Args:
        ...
        config: Model config.json dict **or** a QuantizationConfig instance.
    ...
    non_quantizable = (
        config.non_quantizable
        if isinstance(config, QuantizationConfig)
        else config.get("_oq_non_quantizable", set())
    )
    ...
    tc = (
        config.text_config
        if isinstance(config, QuantizationConfig)
        else config.get("text_config", {})
    )
    num_layers = (
        config.num_hidden_layers
        if isinstance(config, QuantizationConfig)
        else (config.get("num_hidden_layers") or tc.get("num_hidden_layers", 32))
    )
    num_experts = (
    ...
    hidden_size = (
        config.hidden_size
        if isinstance(config, QuantizationConfig)
        else (config.get("hidden_size") or tc.get("hidden_size", 0))
    )
    ...
    boost_map = (
        config.boost_map
        if isinstance(config, QuantizationConfig)
        else config.get("_oq_boost_map") or {}
    )
    if path in boost_map:
        return dict(boost_map[path])

    if isinstance(config, QuantizationConfig):
        if config.use_budget_plan:
            if any(p in path for p in ("ssm_output", "ssm_out")):
                return bits(8)
            if "lora.2" in path:
                return bits(8)
            return True
    elif config.get("_oq_use_budget_plan"):
        if any(p in path for p in ("ssm_output", "ssm_out")):
            return bits(8)
        if "lora.2" in path:
            return bits(8)
    ...
    sensitivity_map = (
        config.sensitivity_map
        if isinstance(config, QuantizationConfig)
        else config.get("_oq_sensitivity_map")
    )
    ...
    if any(p in path for p in ("down_proj", "w2", "mlp.fc2", "wo")):
        is_routed_expert = is_moe and "switch_mlp" in path or "experts" in path
        if is_routed_expert:
            if oq_level == 3.5:
                return bits(4)
            return True
    ...
    if any(p in path for p in ("qkv_proj", "in_proj_qkv")):
        if sensitive:
            return bits(5)
...
def _build_quant_plan(
    ...
    layer_scores = config.sensitivity_map
    ...
    module = None
    for path, shape in named_shapes.items():
        pred = universal_quant_predicate(path, module, config, oq_level)
        if pred is False:
            continue
    ...
    for path, shape in named_shapes.items():
        ...
        if _is_routed_expert(path):
            continue
        floor_pred = universal_quant_predicate(path, module, config, oq_level)
    ...
    # Sensitivity-based greedy boost
    candidates = []
    for path, shape in named_shapes.items():
        if _is_routed_expert(path):
            continue
...
def estimate_bpw_and_size(
    ...
    if _level_targets is not None:
        config_sensitivity_map: dict[str, float] = {}
        num_layers = config.num_hidden_layers or (
            config.text_config.num_hidden_layers if config.text_config else 32
        )
        ...
        for i in range(num_layers):
            if i < num_layers // 8 or i >= 7 * num_layers // 8:
                ...
                pos_sens[str(i)] = 0.01
        config = QuantizationConfig(
            sensitivity_map={**config.sensitivity_map, **pos_sens},
            use_budget_plan=True,
        )
        ...
        config = QuantizationConfig(
            sensitivity_map=config.sensitivity_map,
            boost_map=plan.boost_map,
        )
    else:
        config = QuantizationConfig(
            sensitivity_map=config.sensitivity_map,
